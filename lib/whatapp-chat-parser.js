const WhatsAppChatParser = () => {
    ...
    */
    ...
    const parseMessages = (messages, files) => {
        ...
        const linkRegex = RegExp('(http\:\/\/|https\:\/\/)?([a-z0-9][a-z0-9\-]*\.)+[a-z0-9][a-z0-9\-]*');
        ...
        const parseMessageBody = (body) => {
            ...
            if(linkRegexResult != null) {
                const [link] = linkRegexResult;
                link

            }
            */
           return filename != undefined ? {text, filename} : {text} 
        };
